	...
	idx = (int*)malloc(sizeof(int)* num); // to record the neighborhood corner point should be deleted
		if (num == 0){ // the first point 
			...
			for (i = 0; i<num; i++){
				...
				if (sqrt(pow(curr_point.x - corner[i].x, 2.0) + pow(curr_point.y-corner[i].y, 2.0)) < EUC_DISTANCE){
					isNeighbor = 1;
					if (corner_cost[i] < curr_cost) // more accurate corner detected
						idx[idxnum++] = i;
				}
			}
			if (idxnum > 0){
...
int DetectCorner(IplImage *img, CvPoint *corner){
	...
	for (i = B_SIZE; i<height - B_SIZE; i++)
		for (j = B_SIZE; j<width - B_SIZE; j++){
			...
			// SVD The flags cause U and V to be returned transposed (does not work well without the transpose flags).
			...
			if (num >= MAX_CORNERPOINT_NUM){
				cout << "error. MAX_CORNERPOINT_NUM reached!"<<endl;
				return-1;
			}
		}
	cvReleaseMat(&G);
...
//********************************************************************** 
// finding the normalization matrix x' = T*x, whereT={s,0,tx, 0,s,ty, 0,0,1} 
...
void RANSAC_homography(int num, CvPoint2D64f *m1, CvPoint2D64f *m2, CvMat *H,
	...
	// RANSAC algorithm (reject outliers and obtain thebest H)  
	srand(134);
	MAX_num = -1;
	while (N > sample_cnt){
...
int mmain(int argc, char*argv[])
{
	...
	// detect corner 
	// corner points are stored in CvPoint cornerp1 andcornerp2 
				...
				if (count != 0){
					for (k = 0; k<channels; k++){
						data_interp[i*step + j*channels + k] =
							(int)((data_scene[(i - 1)*step + j*channels + k] + data_scene[(i + 1)*step + j*channels + k] + data_scene[i*step + (j - 1)*channels + k] + data_scene[i*step + (j + 1)*channels + k]) / count);
						msetmp += pow(data_img1[i*step + j*channels + k] - data_interp[i*step + j*channels + k], 2.0);
					}
					mse += msetmp / channels;
					pixnum++;
				}
	...
	{
		float* pptr = (float*)((*H).data.ptr + row * (*H).step);//第row行数据的起始指针
		for (int col = 0; col < (*H).cols; col++)
		{
			//cout << *(pptr + 1 * col) <<"  ";
			cout << cvmGet(H, row, col) << "\t";
		}
		cout << endl;
	}
	...
	cvInvert(H, invH);
	cout << endl; cout << endl;
	for (int row = 0; row < (*invH).rows; row++)
	{
		float* pptr = (float*)((*invH).data.ptr + row * (*invH).step);//第row行数据的起始指针
		for (int col = 0; col < (*invH).cols; col++)
		{
			//cout << *(pptr + 1 * col) <<"  ";
			cout << cvmGet(invH, row, col) << "\t";
		}
		cout << endl;
	}
	img_scene = cvCloneImage(img_2);
